...
import { fetchLightroomPhotos, getFeaturedPhoto } from '@/lib/lightroom';
  ...
  const previews = await Promise.all(
    files
      ...
      .map(async (file) => {
        ...
        // Fetch photos and get featured cover image
        const photos = await fetchLightroomPhotos(config.lightroomShareId);
        const featuredPhoto = getFeaturedPhoto(photos);
        const coverImageUrl = featuredPhoto?.url640 || '';
